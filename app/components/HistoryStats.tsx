      ...
      <div className="stack stagger">
        <section className="stats" aria-label="Ringkasan hasil sinyal">
          <div className="stat-card stat-card-wide">
            ...
            <div className="strip" role="img" aria-label={`Hasil ${strip.length} sinyal terakhir dari yang terlama ke terbaru`}>
              {strip.map((s) => (
                <span
                  key={s.id}
                  className={`strip-cell strip-${outcomeTone(s.outcome_wide)}`}
                  title={`${s.symbol} ${outcomeLabel(s.outcome_wide)} ${s.pnl_wide_pct !== null ? formatPct(s.pnl_wide_pct) : ""}`}
                />
              ))}
            </div>
          ...
          <div className="stat-card">
            ...
            <Sparkline stretch height={34} width={200} values={cumulative} label="Kurva P&L kumulatif" className={`tone-${pctTone(totalPnl)}`} />
          </div>
        </section>
